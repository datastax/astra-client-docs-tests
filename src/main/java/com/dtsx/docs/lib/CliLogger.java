...
public class CliLogger {
    ...
    public static void result(TestRoot testRoot, TestOutcome testOutcome, String snapshot) {
        if (testOutcome.passed()) {
            return;
        }

        accumulated.add("[FAILED] " + testRoot.rootName() + " -> " + testOutcome.name());

        if (testOutcome instanceof TestOutcome.Failed(Optional<Path> expected)) {
            expected.ifPresentOrElse((path) -> {
                accumulated.add("Expected snapshot file: " + path);
            }, () -> {
                accumulated.add("No previously approved snapshot file found.");
            });

            accumulated.add("Snapshot:\n" + snapshot);
        } else if (testOutcome instanceof TestOutcome.Errored(Exception error)) {
            accumulated.add(captureStackTrace(error));
        }
    }
